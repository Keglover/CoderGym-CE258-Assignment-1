...
LR = 0.0001             # Learning rate reduced for RMSProp to account for its adaptive nature
EPOCHS = 50             # Some more epochs to show the convergence changes more
BETAS = (0.9, 0.999)    # Common default values for Adam's moving average factors - here for disambiguation and clarity
...
# The optimizer we are to implement for this task - Adam
class ManualOptimizer:
    def __init__(self, params, lr, betas=BETAS, eps=1e-8):
        self.params = list(params)
        self.lr = lr
        self.beta_first = betas[0]
        self.beta_second = betas[1]
        self.eps = eps
        self.avg_grad = [torch.zeros_like(param, device=param.device) for param in self.params]
        self.sqd_avg = [torch.zeros_like(param, device=param.device) for param in self.params]
        self.timestep = 0
    ...
    def step(self):
        self.timestep += 1
        with torch.no_grad():
            for param, sqd_avg, avg_grad in zip(self.params, self.sqd_avg, self.avg_grad):
                if param.grad is not None:
                    if avg_grad.device != param.device:
                        avg_grad = avg_grad.to(param.device)
                    if sqd_avg.device != param.device:
                        sqd_avg = sqd_avg.to(param.device)

                    avg_grad.mul_(self.beta_first).add_((1 - self.beta_first) * param.grad)
                    grad_bias = avg_grad / (1 - self.beta_first ** self.timestep)

                    sqd_avg.mul_(self.beta_second).add_((1 - self.beta_second) * param.grad ** 2)
                    sqd_bias = sqd_avg / (1 - self.beta_second ** self.timestep)

                    param.data.sub_((self.lr / (sqd_bias.sqrt() + self.eps)) * grad_bias)
...
# AdamW with PyTorch's built-in implementation
# =================================================================================
...
def main():
    ...
    opt_pt      = adamw(model_pt.parameters(), lr=LR, betas=BETAS, eps=EPS)
    ...
    opt_ex     = ManualOptimizer(model_ex.parameters(), lr=LR, betas=BETAS, eps=EPS)
    ...
    loss_close = abs(val_metrics_pt['val_loss'] - val_metrics_ex['val_loss']) < 0.15
    ...
    print(f"✓ Manual SGD val loss close to PyTorch (<0.15): {abs(val_metrics_pt['val_loss'] - val_metrics_ex['val_loss']):.6f}" if loss_close else f"✗ Manual SGD val loss not close enough: {abs(val_metrics_pt['val_loss'] - val_metrics_ex['val_loss']):.6f}")
