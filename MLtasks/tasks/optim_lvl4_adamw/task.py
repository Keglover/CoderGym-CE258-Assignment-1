...
class ManualOptimizer:
    def __init__(self, params, lr, beta_first=0.9, beta_second=0.999, eps=1e-8):
        self.params = list(params)
        self.lr = lr
        self.beta_first = beta_first
        self.beta_second = beta_second
        self.eps = eps
        self.avg_grad = [torch.zeros_like(param) for param in self.params]
...
def main():
    ...
    opt_pt      = adam(model_pt.parameters(), lr=learning_rate)
    ...
    print(f"PyTorch Adam final train loss: {history_pt['Train Loss'][-1]:.4f}")
    print(f"Experiment Adam final train loss: {history_ex['Train Loss'][-1]:.4f}")

    print(f"\nPyTorch Adam final val loss: {history_pt['Val Loss'][-1]:.4f}")
    print(f"Experiment Adam final val loss: {history_ex['Val Loss'][-1]:.4f}")

    print(f"\nPyTorch Adam final val acc: {history_pt['Val Acc'][-1]:.4f}")
    print(f"Experiment Adam final val acc: {history_ex['Val Acc'][-1]:.4f}")

    print(f"\nPyTorch Adam time elapsed: {history_pt['Time Elapsed']:.2f} seconds")
    print(f"Experiment Adam time elapsed: {history_ex['Time Elapsed']:.2f} seconds")
