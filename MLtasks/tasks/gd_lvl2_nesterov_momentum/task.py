    ...
    def step(self):
        with torch.no_grad():
            for param, velocity in zip(self.params, self.velocity):
                if velocity.device != param.device:
                    velocity = velocity.to(param.device)

                if param.grad is not None:
                    velocity.mul_(self.momentum).sub_(self.lr * param.grad)             # Need the leading _ in the function names - tells Python it's an in-place operation
                    param.data.add_(self.momentum * velocity - self.lr * param.grad)
...
# SGD + Nesterov Momentum with PyTorch's built-in implementation
# =================================================================================
...
def main():
    ...
    opt_pt      = torchSGD(model_pt.parameters(), lr=LR, momentum=MOMENTUM_FACTOR, nesterov=True)
    ...
    opt_ex     = ManualOptimizer(model_ex.parameters(), lr=LR, momentum=MOMENTUM_FACTOR)
    ...
    loss_close = abs(val_metrics_pt['val_loss'] - val_metrics_ex['val_loss']) < 0.15 # Looser threshold to account for higher variance than in vanilla SGD
